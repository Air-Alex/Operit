...
private const val DEFAULT_ANDROID_PROJECT_CONFIG_JSON = """
{
    ...
    "commands": [
        ...
        },
        {
            ...
            "label": "⬇️ 下载并配置 SDK",
            "command": "SDK_DIR=\"${'$'}HOME/android/sdk\"; if [ -d \"${'$'}SDK_DIR/platform-tools\" ]; then echo '✅ 检测到已安装的 SDK'; echo \"SDK位置: ${'$'}SDK_DIR\"; echo \"sdk.dir=${'$'}SDK_DIR\" > local.properties; echo 'local.properties 已配置'; else echo '开始安装 Android SDK...'; ARCH=$(uname -m); if [ \"${'$'}ARCH\" = \"aarch64\" ] || [ \"${'$'}ARCH\" = \"arm64\" ]; then echo \"检测到 ARM64 架构，将安装 ARM64 版本工具\"; mkdir -p \"${'$'}SDK_DIR\" && cd \"${'$'}SDK_DIR\" && echo '正在下载 Command Line Tools...' && wget -q https://dl.google.com/android/repository/commandlinetools-linux-11076708_latest.zip -O cmdtools.zip && echo '正在解压...' && unzip -q cmdtools.zip && rm cmdtools.zip && mkdir -p cmdline-tools/latest && mv cmdline-tools/* cmdline-tools/latest/ 2>/dev/null || true && echo '正在安装 SDK 组件 (约500MB)...' && yes | cmdline-tools/latest/bin/sdkmanager --sdk_root=\"${'$'}SDK_DIR\" 'platform-tools' 'platforms;android-34' 'build-tools;34.0.0' && cd - && echo \"sdk.dir=${'$'}SDK_DIR\" > local.properties && echo '✅ Android SDK 安装完成！' && echo \"SDK位置: ${'$'}SDK_DIR\" && echo 'local.properties 已配置'; else echo '⚠️ 非 ARM64 架构，请手动配置 SDK'; fi; fi",
            "workingDir": ".",
            "shell": true
        },
        {
